...
import torch
import numpy as np
from Bertstat.bert.modeling import BERTLayerNorm
...
def compute_Linear_flops(module, inp, out):
    # inp:bs*m*n, out: bs*m*n flops=bs*2mnp
    assert isinstance(module, nn.Linear)
    batch_size = inp.size()[0] if len(inp.size()) == 3 else 1
    return batch_size * inp.size()[-2] * 2 * inp.size()[-1] * out.size()[-1]
...
def compute_LayerNorm_flops(module, inp, out):
    batch_flops = np.prod(inp.shape)
    return batch_flops * 2


def compute_Embedding_flops(module, inp, out):
    weight = module.weight
    batch_size = inp.size()[0]
    return batch_size * inp.size()[1] * np.prod(weight.shape) * 2
